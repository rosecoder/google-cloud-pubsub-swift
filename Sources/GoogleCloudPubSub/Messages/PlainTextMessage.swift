...
extension PublisherProtocol {
  ...
  public func publish(
    ...
    (try await publish(
      to: topic, bodies: [body], attributes: attributes, file: file, function: function, line: line))[
        0]
  }
  ...
  public func publish(
    ...
    let messages = try bodies.map {
      try PlainTextMessage.Outgoing(body: $0, attributes: attributes)
    }
    return try await publish(
      to: topic, messages: messages, file: file, function: function, line: line)
  }
}
...
public struct IncomingPlainTextMessage: IncomingMessage {
  ...
  public init(
    id: String, published: Date, data: Data, attributes: [String: String], logger: inout Logger,
    span: any Span
  ) throws {
